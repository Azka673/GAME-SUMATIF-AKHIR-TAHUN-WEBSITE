...
class NPC {
  ...
  getDialogue(context = {}) {
    ...
    if (this.panicking) {
      return {
        text: `Tolong! Aku kehilangan ${itemName}... Terakhir kali aku ingat ada di dekat ${this.getLastSeenClue()}. Ini sangat penting! 😭`,
        choices: [
          { text: 'Aku akan cari untukmu!', action: 'accept_quest' },
          { text: 'Bagaimana kalau barangmu sudah hilang?', action: 'lost_check' },
          { text: 'Ada petunjuk lain?', action: 'location_clue' },
          { text: 'Nanti dulu...', action: 'close' },
        ],
      };
    }

    if (this.isTeacher && this.canGiveQuest) {
      const difficulty = this.floor === 0 ? 'mudah' : this.floor === 1 ? 'sedang' : 'sulit';
      return {
        text: `Selamat datang, Returner. Saya ${this.name}. Banyak barang hilang di sekolah ini — tolong bantu kembalikan ke pemiliknya. Tugas kali ini tingkat kesulitan: ${difficulty}.`,
        choices: [
          { text: 'Saya siap, Pak/Bu!', action: 'teacher_quest' },
          { text: 'Barang apa saja yang hilang di sini?', action: 'teacher_hint' },
          { text: 'Ada hadiah menarik?', action: 'close' },
    ...
    if (!this.hasQuest && !this.isTeacher) {
      const moods = ['santai', 'bosan', 'sibuk', 'senang', 'khawatir'];
      const mood = moods[this.floor % moods.length];
      return {
        text: `Hai, aku ${this.name}. Lagi ${mood} nih... Oh ya, ada yang aku lihat tadi! Mungkin bisa membantu pencarian barang.`,
        choices: [
          { text: 'Kamu tadi lihat apa?', action: 'ask_witness' },
          { text: 'Ada barang milikmu yang hilang?', action: 'lost_check' },
          { text: 'Baik, sampai jumpa!', action: 'close' },
        ],
      };
    }
    ...
    if (context.askingWitness && context.item) {
      ...
      const time = ['pagi', 'siang', 'sore', 'malam'][Math.floor(Math.random() * 4)];
      return {
        text: `Hmm... terakhir kali aku melihat barang itu di ${room?.name || 'suatu tempat'} di lantai ${(context.item.floor || 0) + 1}. Aku lihat pas waktu ${time} kok!`,
        choices: [
          { text: 'Terima kasih infonya! Sangat membantu!', action: 'add_witness_clue', item: context.item },
          { text: 'Ada informasi lain?', action: 'close' },
          { text: 'Oke deh', action: 'close' },
        ],
      };
    }

    const lines = [
      `Hai! Aku ${this.name}. Kamu dari organisasi Returners ya? Reputasi kalian bagus di sekolah! 😊`,
      `Di sekolah ini sering ada barang hilang. Tadi aku lihat ada yang cemas mencari sesuatu...`,
      `Kalau ada yang panik, mereka biasanya bisa kasih petunjuk yang jelas.`,
      `Organisasi kalian keren! Banyak yang terbantu akhir-akhir ini.`,
      `Semoga cepat ketemu barangnya! Orang-orang jadi lebih rileks kalau barangnya ketemu.`,
    ];
    return {
      text: lines[this.dialogueIndex % lines.length],
      choices: [
        { text: 'Ada yang kehilangan barang hari ini?', action: 'lost_check' },
        { text: 'Tanya petunjuk lokasi barang hilang', action: 'ask_witness' },
