...
export class SaveSystem {
  ...
  save() {
    const data = {
      ...
      timelineDistortion: this.game.timeline?.distortionLevel ?? 0,
    };
    try {
  ...
  load() {
    try {
      ...
      this.game.player.x = data.player?.x ?? 600;
      this.game.player.y = data.player?.y ?? 400;
      ...
      if (this.game.skillTree) {
        this.game.skillTree.skills = data.skills ?? {};
      ...
      if (this.game.timeline) this.game.timeline.distortionLevel = data.timelineDistortion ?? 0;
      return true;
    } catch (e) {
