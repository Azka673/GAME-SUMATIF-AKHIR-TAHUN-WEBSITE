...
export class MainMenu {
  constructor(game) {
    this.game = game;
    // Ensure DOM elements are loaded with null checks
    this.loadingScreen = document.getElementById('loading-screen') || null;
    this.mainMenu = document.getElementById('main-menu') || null;
    this.menuBg = document.getElementById('menu-bg') || null;
    this.loadingFill = document.getElementById('loading-fill') || null;
    this.loadingText = document.getElementById('loading-text') || null;
    
    if (!this.loadingScreen || !this.mainMenu || !this.loadingText || !this.loadingFill) {
      console.error('❌ Loading screen DOM elements not found!', {
        loadingScreen: !!this.loadingScreen,
        mainMenu: !!this.mainMenu,
        loadingText: !!this.loadingText,
        loadingFill: !!this.loadingFill,
      });
    }
  ...
  async showLoading() {
    ...
    // Ensure main menu is hidden initially
    if (this.mainMenu) {
    ...
    try {
      ...
      await this._transitionToMainMenu();
      
    } catch (error) {
      console.error('❌ Error in loading screen:', error);
      // Fallback: force transition to main menu
      this._forceMainMenuDisplay();
    }
  }

  async _transitionToMainMenu() {
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        ...
        if (this.loadingScreen) {
          ...
          this.loadingScreen.style.opacity = '1'; // Reset for potential reuse
  ...
  _forceMainMenuDisplay() {
    console.warn('⚠️ Forcing main menu display');
    if (this.loadingScreen) {
      this.loadingScreen.style.display = 'none';
      this.loadingScreen.classList.remove('active');
    }
