...
export class TimelineSystem {
  ...
  _applyDistortion() {
    const effects = [
      'Seorang murid tidak ada di daftar kelas hari ini...',
      'Jadwal sekolah berubah mendadak...',
      'Cuaca di luar sekolah menjadi aneh...',
      'Lebih banyak barang hilang di koridor!',
    ];
    const e = effects[Math.floor(Math.random() * effects.length)];
    this.game.ui?.notify(e, 'danger');
    if (e.includes('barang hilang')) {
      this.game.npcManager?._spawnLostItem?.(true);
    }
