      ...
      if (room.type === 'classroom') {
        for (let row = 0; row < 3; row++) {
          for (let col = 0; col < 4; col++) {
            dec.push({ floor: room.floor, x: room.x + 40 + col * 55, y: room.y + oy + 60 + row * 55, type: 'desk' });
          }
        }
        dec.push({ floor: room.floor, x: room.x + room.w / 2 - 40, y: room.y + oy + 30, type: 'board' });
      }
  ...
  _drawDecoration(ctx, d, dx, dy) {
    switch (d.type) {
      case 'desk':
        ...
        ctx.fillRect(dx, dy, 44, 32);
        ctx.fillStyle = '#4a5570';
        ctx.fillRect(dx + 4, dy + 4, 36, 20);
        break;
      case 'board':
