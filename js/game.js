 ...
 * Proper state management and pause system implemented
 */
...
import { GameStateManager, globalGameState } from './gameState.js';
import { ParticleSystem } from './particleSystem.js';
import { ItemManager } from './itemSystem.js';
import { MobileJoystick } from './mobileJoystick.js';
import { DayNightCycleSystem } from './dayNightCycle.js';
...
class Game {
  constructor() {
    ...
    this.mobileJoystick = new MobileJoystick(this);
    this.dayNightCycle = new DayNightCycleSystem(this);
    this.objectives = new ObjectiveSystem(this);
    this.world.currentLocation = 'school'; // Start at school
  }
  ...
  _bindInput() {
    ...
    window.addEventListener('keydown', e => {
      ...
      switch (e.code) {
        ...
        case 'KeyL': this.objectives.toggle(); break;
        case 'KeyQ': {
  ...
  start(loadSave = false) {
    this.running = true;
    globalGameState.setState('PLAYING');
    this.audio.init();
  ...
  loop(timestamp) {
    // Game loop ALWAYS continues for rendering
    if (!this.running) return;
    
    const dt = Math.min((timestamp - this.lastTime) / 1000, 0.05);
    this.lastTime = timestamp;
    
    // Only call update if NOT paused and NOT in dialogue
    // This is the KEY FIX: update is skipped during pause, but render continues
    if (!this.dialogue.active && !this.paused && globalGameState.isPlaying()) {
      this.update(dt);
    }
    
    // Render always happens, regardless of pause state
    this.render();
    
    // RequestAnimationFrame continues - game loop never stops
    requestAnimationFrame(t => this.loop(t));
  }

  update(dt) {
    const scaledDt = dt * this.timeScale;
    this.mobileJoystick?.update();
    this.hour += scaledDt * 0.03;
    if (this.hour >= 24) { this.hour = 0; this.day++; }
    this.dayNightCycle?.update(scaledDt, this.hour);
    ...
    this.itemManager.update(scaledDt);
  ...
  render() {
    ...
    this.itemManager.draw(ctx, cam.x, cam.y, this.player.floor);
    ...
    if (this.skillTree.skills.scanClue) {
      this.itemManager.getAllItemsAt(this.player.floor)
        .forEach(item => {
    ...
    if (this.skillTree.skills.fastAnalysis) {
      const center = this.player.getCenter();
      const nearItem = this.itemManager.items.find(i =>
        i.floor === this.player.floor &&
        !i.found &&
        Math.hypot(i.x - center.x, i.y - center.y) < 200
      );
      if (nearItem?.ownerId) {
        const owner = this.npcManager.npcs.find(n => n.id === nearItem.ownerId);
        if (owner) {
